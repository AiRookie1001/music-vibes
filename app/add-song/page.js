...
function LoadingState() {
  ...
  return (
    <div style={{
      ...
      <WaveAnimation />
      <div>
        <p style={{ fontSize: '14px', color: '#f2f2f2', fontWeight: '600', textAlign: 'center', marginBottom: '6px' }}>
          {LOADING_MESSAGES[msgIndex]}{dots}
        </p>
        <p style={{ fontSize: '12px', color: '#444', textAlign: 'center' }}>
          This usually takes 20–40 seconds
...
export default function AddSong() {
  ...
  const [errorMsg, setErrorMsg]           = useState('')
  const [touched, setTouched]             = useState({ url: false, title: false, artist: false })
  const [duplicateSong, setDuplicateSong] = useState(null)   // { title, artist } if found
  ...
  // ── Debounced duplicate check — runs 600ms after user stops typing ──────────
  useEffect(() => {
    setDuplicateSong(null)
    if (!ytId) { setCheckingDupe(false); return }
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        const { data } = await supabase
          .from('songs')
          .select('title, artist')
          .ilike('audio_url', `%${ytId}%`)
          .maybeSingle()
        setDuplicateSong(data ?? null)
      } catch {
        // Silent fail — don't block upload over a check error
      } finally {
  ...
  const errors = {
    url:    !youtubeUrl.trim() ? 'YouTube URL is required'
          : !isValidYt         ? 'Enter a valid YouTube URL'
          : duplicateSong      ? `Already in library as "${duplicateSong.title}" by ${duplicateSong.artist}`
          : null,
    title:  !title.trim()  ? 'Song title is required'  : null,
    artist: !artist.trim() ? 'Artist name is required' : null,
  }
  const hasErrors    = Object.values(errors).some(Boolean)
  const isBlocked    = !!duplicateSong || checkingDupe

  const handleSubmit = async () => {
    setTouched({ url: true, title: true, artist: true })
    if (hasErrors || isBlocked) return

    // Final guard — re-check right before sending to API
    if (ytId) {
      const { data: existing } = await supabase
        ...
        .ilike('audio_url', `%${ytId}%`)
        .maybeSingle()
      if (existing) { setDuplicateSong(existing); return }
    }
  ...
  // ── URL field border colour ───────────────────────────────────────────────
  const urlBorderColor = () => {
    if (touched.url && errors.url) return '#ff4444'           // red   — invalid / duplicate
    if (checkingDupe)              return '#444'              // grey  — checking
    if (ytId && !duplicateSong)    return '#2a7a3a'           // green — valid & unique
    return '#2e2e2e'                                          // default
  }

  const inputStyle = (key) => ({
    width: '100%', padding: '12px 16px', boxSizing: 'border-box',
    backgroundColor: '#181818',
    border: `1px solid ${key === 'url' ? urlBorderColor() : (touched[key] && errors[key] ? '#ff4444' : '#2e2e2e')}`,
    borderRadius: '8px', color: '#f2f2f2', fontSize: '14px',
    outline: 'none', fontFamily: 'DM Sans, sans-serif', transition: 'border-color 0.15s',
  })

  const labelStyle = {
    fontSize: '12px', color: '#888', display: 'block', marginBottom: '8px',
  ...
  // Small hint under URL field
  const urlHint = () => {
    if (!ytId) return null
    if (checkingDupe) return <p style={{ fontSize: '11px', color: '#555', marginTop: '6px' }}>⏳ Checking for duplicates...</p>
    if (!duplicateSong && !(touched.url && errors.url)) return <p style={{ fontSize: '11px', color: '#2a7a3a', marginTop: '6px' }}>✓ URL looks good</p>
    return null
  }

  // Submit button label + colour
  const btnLabel = checkingDupe ? '⏳ Checking...' : duplicateSong ? '🚫 Already in Library' : '+ Upload Song'
  const btnActive = !hasErrors && !isBlocked
  const btnStyle = {
    padding: '13px', border: 'none', borderRadius: '8px',
    backgroundColor: btnActive ? '#ff5500' : '#222',
  ...
  if (status === 'success') return (
    <div style={{ padding: '40px 28px', maxWidth: '520px' }}>
      <div style={{
        ...
        <div style={{ fontSize: '40px', marginBottom: '12px' }}>✅</div>
        <h2 style={{ fontSize: '18px', fontWeight: '700', color: '#69db7c', marginBottom: '6px' }}>Song added!</h2>
        <p style={{ color: '#555', fontSize: '13px' }}>Redirecting to your library...</p>
      </div>
    </div>
  )
    ...
    <div style={{ padding: '40px 28px', maxWidth: '520px' }}>
      ...
      {status === 'loading' ? <LoadingState /> : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          {/* YouTube URL */}
          <div>
            <label style={labelStyle}>YouTube URL <span style={{ color: '#ff5500' }}>*</span></label>
            <input
              type="text"
              placeholder="https://youtube.com/watch?v=..."
              value={youtubeUrl}
              ...
              style={inputStyle('url')}
            />
            {/* Show duplicate error OR inline hint, never both */}
            {touched.url && errors.url ? fieldError('url') : urlHint()}
          </div>
          ...
          {duplicateSong && (
            <div style={{
              padding: '14px 16px', borderRadius: '8px',
              backgroundColor: '#1e1010', border: '1px solid rgba(255,100,100,0.25)',
              display: 'flex', gap: '12px', alignItems: 'flex-start',
            }}>
              <span style={{ fontSize: '18px', flexShrink: 0 }}>🚫</span>
              <div>
                <p style={{ fontSize: '13px', color: '#ff6b6b', fontWeight: '600', marginBottom: '3px' }}>
                  This song is already in your library
                </p>
                <p style={{ fontSize: '12px', color: '#666' }}>
                  <strong style={{ color: '#999' }}>{duplicateSong.title}</strong> by {duplicateSong.artist}
                </p>
              </div>
            </div>
          )}
          ...
          {/* Song Title */}
          <div>
            <label style={labelStyle}>Song Title <span style={{ color: '#ff5500' }}>*</span></label>
            <input
              type="text"
              placeholder="e.g. Blinding Lights"
              value={title}
          ...
          {/* Artist */}
          <div>
            <label style={labelStyle}>Artist Name <span style={{ color: '#ff5500' }}>*</span></label>
            <input
              type="text"
              placeholder="e.g. The Weeknd"
              value={artist}
          ...
          <button
            ...
            disabled={isBlocked}
            style={btnStyle}
            onMouseEnter={e => { if (btnActive) e.currentTarget.style.backgroundColor = '#ff7733' }}
            onMouseLeave={e => { if (btnActive) e.currentTarget.style.backgroundColor = '#ff5500' }}
          >
            {btnLabel}
          </button>
          ...
          {/* Supported formats */}
          <div style={{ padding: '14px 16px', borderRadius: '8px', backgroundColor: '#141414', border: '1px solid #1e1e1e' }}>
            <p style={{ fontSize: '11px', color: '#444', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '8px' }}>
              Supported URLs
            </p>
            {['youtube.com/watch?v=...', 'youtu.be/...', 'youtube.com/shorts/...'].map(ex => (
              <p key={ex} style={{ fontSize: '12px', color: '#3a3a3a', fontFamily: 'DM Mono, monospace', marginTop: '4px' }}>✓ {ex}</p>
            ))}
          </div>
